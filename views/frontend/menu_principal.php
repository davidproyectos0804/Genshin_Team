<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <script src="https://cdn.tailwindcss.com"></script>
  <title>Genshin TeamBuilder | Inicio</title>
  <link rel="stylesheet" href="../../public/assets/css/menu_principal.css">
</head>

<body class="text-white flex flex-col">

  <?php include 'reusables/nav.html'; ?>

  <main class="flex-1 flex flex-col items-center justify-center px-6 text-center z-10">

    <!-- Tag -->
    <div class="mb-4 px-3 py-1 border border-indigo-500/20 bg-indigo-500/5 rounded-full text-indigo-400/80 text-[8px] font-black tracking-[0.4em] uppercase">
      Bienvenido
    </div>

    <!-- Title -->
    <h1 class="text-6xl md:text-[6.5rem] font-black tracking-tighter leading-none mb-4 title-gradient uppercase">
      Genshin Builder
    </h1>

    <div class="h-1 w-24 bg-indigo-500/30 mx-auto rounded-full mb-8"></div>

    <!-- Description -->
    <p class="text-slate-300 text-base md:text-xl max-w-3xl font-medium leading-relaxed mb-12 opacity-90 px-4">
      Crea tus equipos, explora personajes y sigue el meta de 
      <span class="text-indigo-400 font-bold italic border-b border-indigo-500/20 pb-1">
        Genshin Impact
      </span>.
      <br>
      Optimiza tu composición y domina el Abismo.
    </p>

    <!-- Cards -->
    <div class="grid md:grid-cols-3 gap-8 w-full max-w-5xl">

      <!-- Personajes -->
      <a href="index.php?controlador=personajes&accion=cMostrarPersonajes"
         class="group p-8 bg-black/50 border border-white/5 rounded-2xl backdrop-blur-xl transition-all duration-300 hover:border-indigo-500/40 hover:bg-indigo-900/10">

        <h3 class="text-2xl font-black mb-2 uppercase group-hover:text-indigo-400">
          Personajes
        </h3>

        <p class="text-slate-500 text-[10px] font-bold tracking-[2px] uppercase">
          Stats • Builds
        </p>
      </a>

      <!-- TeamBuilder (DESTACADO) -->
      <a href="index.php?controlador=personajes&accion=cMostrarTeambuilder"
         class="group p-10 bg-black/60 border border-indigo-500/20 rounded-2xl backdrop-blur-xl transition-all duration-300 hover:border-indigo-400 hover:bg-indigo-900/20 scale-105">

        <h3 class="text-3xl font-black mb-2 uppercase text-indigo-400 group-hover:text-indigo-300">
          TeamBuilder
        </h3>

        <p class="text-slate-400 text-[10px] font-bold tracking-[2px] uppercase">
          Crea • Optimiza • Comparte
        </p>
      </a>

      <!-- Banners -->
      <a href="#"
         class="group p-8 bg-black/50 border border-white/5 rounded-2xl backdrop-blur-xl transition-all duration-300 hover:border-purple-500/40 hover:bg-purple-900/10">

        <h3 class="text-2xl font-black mb-2 uppercase group-hover:text-purple-400">
          Banners
        </h3>

        <p class="text-slate-500 text-[10px] font-bold tracking-[2px] uppercase">
          Rotaciones • Probabilidades
        </p>
      </a>

    </div>

  </main>

  <?php include 'reusables/fotter.html'; ?>

</body>
</html>